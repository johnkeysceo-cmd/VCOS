...
const SettingsView = ({
  ...
  useEffect(() => {
    const setDefaultPath = async () => {
      try {
        // Check for VCOS export directory first (set by VCOS launcher)
        const vcosExportDir = await window.electronAPI.getEnv('VCOS_EXPORT_DIR')
        if (vcosExportDir) {
          const filename = generateFilename(settings.format)
          setOutputPath(`${vcosExportDir}/${filename}`)
          return
        }

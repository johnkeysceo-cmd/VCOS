...
async function startActualRecording(
  ...
  // Check for VCOS output directory (set by VCOS launcher)
  const vcosOutputDir = process.env.VCOS_OUTPUT_DIR || process.env.SCREENARC_OUTPUT_DIR
  const recordingDir = vcosOutputDir 
    ? path.resolve(vcosOutputDir)
    : path.join(process.env.HOME || process.env.USERPROFILE || '.', '.screenarc')
...
export async function cleanupOrphanedRecordings() {
  ...
  // Check for VCOS output directory
  const vcosOutputDir = process.env.VCOS_OUTPUT_DIR || process.env.SCREENARC_OUTPUT_DIR
  const recordingDir = vcosOutputDir 
    ? path.resolve(vcosOutputDir)
    : path.join(process.env.HOME || process.env.USERPROFILE || '.', '.screenarc')

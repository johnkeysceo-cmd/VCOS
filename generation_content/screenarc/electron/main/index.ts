...
// Check for VCOS integration environment variables
const VCOS_OUTPUT_DIR = process.env.VCOS_OUTPUT_DIR || process.env.SCREENARC_OUTPUT_DIR
const VCOS_AUTO_LAUNCH = process.env.VCOS_AUTO_LAUNCH === 'true'

app.whenReady().then(async () => {
  log.info('[App] Ready. Initializing...')
  
  // If VCOS output directory is set, configure it
  if (VCOS_OUTPUT_DIR) {
    log.info(`[VCOS] Output directory configured: ${VCOS_OUTPUT_DIR}`)
    // Store in app state for use in recording manager
    if (!appState.vcosConfig) {
      appState.vcosConfig = {}
    }
    appState.vcosConfig.outputDirectory = VCOS_OUTPUT_DIR
  }
